...
def menu(my_name,my_id):
	...
	gh = 'github.com/SFFAJAR'
	cetak(nel('\tSelamat Datang  [yellow]%s[white] Anak Kontol'%(my_name)))
...
def crack_file():
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('/sdcard/SFFAJAR_DUMP/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<100:
				nom = ''+str(cih)
		...
		try:geh = lol[geeh]
			...
			time.sleep(3)
			back()
		try:lin = open('/sdcard/SFFAJAR_DUMP/'+geh,'r').read().splitlines()
		except:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r ♻️.py {h}{P}{b}{loop}{P}/{P}{len(id)}{P} {hh}OK:{P}{ok}{P} {kk}CP:{P}{cp}{x}  {P}{'{:.0%}'.format(loop/float(len(id)))}{P}  "),
	sys.stdout.flush()
